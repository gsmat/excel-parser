...
class SIDENOTES_DATA_TABLE extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'excel:side-notes {--index=}';
    ...
    {
        if (Schema::hasTable('INTEGRATION_SIDENOTES_DATA_TABLE') === false) {
            Schema::create('INTEGRATION_SIDENOTES_DATA_TABLE', static function (Blueprint $table) {
        ...
        $path = '/files/SIDENOTES_DATA_TABLE.xlsx';
                    ...
                    try {
                        ...
                        foreach ($this->chunk($data, 349) as $chunk) {
                            ...
                            DB::transaction(function () use ($chunk, $count, $sheetKey) {
                                if (DB::table('INTEGRATION_SIDENOTES_DATA_TABLE')->insert($chunk)) {
